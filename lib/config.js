...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  switch (key) {
    ...
    case 'THEME':
      // Prefer blog.config/env (NEXT_PUBLIC_THEME) over Notion Config
      return convertVal(
  ...
  if (!hasVal(val)) {
    ...
    switch (key) {
      ...
      case 'AVATAR':
        ...
        break
      case 'TITLE':
        // 优先 blog.config.js 中的 TITLE（含 NEXT_PUBLIC_TITLE 环境变量），其次 Notion 标题
        val = BLOG.TITLE || siteInfo?.title
        break
      case 'DESCRIPTION':
        // 优先 blog.config.js 中的 DESCRIPTION，其次 Notion 描述
        val = BLOG.DESCRIPTION || siteInfo?.description
        break
